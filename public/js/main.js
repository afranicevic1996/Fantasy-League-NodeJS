...
document.addEventListener("DOMContentLoaded", function(){
    var deleteClub = document.getElementsByClassName("deleteButton");

    //get all deleteButton elements and attach even listeners
    for(var x = 0; x < deleteClub.length; x++){
        deleteClub[x].addEventListener("click", function(){
            var elementHTML = this.getAttribute("id");
            var path = "/admin/" + this.getAttribute("data-path");
            modalControl("poruka za delete u modalu (auto): " + elementHTML, path, "delete", elementHTML);
        });
    }

    var editClub = document.getElementsByClassName("editButton");

    //get all editButton elements and attach even listeners
    for(var x = 0; x < editClub.length; x++){
        editClub[x].addEventListener("click", function(){
            var elementHTML = this.getAttribute("id");
            var path = "/admin/" + this.getAttribute("data-path");
            modalControl("poruka za edit u modalu (auto): " + elementHTML, path, "edit", elementHTML);
        });
    }

    //login form
    var lf = document.getElementsByClassName("login-form")[0];
    if(lf)
        lf.addEventListener("submit", checkCreds, false);

    //closing server made status message
    var rs = document.getElementsByClassName("renderStatus")[0];
    if(rs){
        setTimeout(function(){
    ...
    //document.getElementById("findPlayers").addEventListener("click", searchPlayers, false);
    var cl = document.getElementById("clubList");
    if(cl)
        cl.addEventListener("change", searchPlayersByClubId, false);

    //adding club to the game UI
    var cs = document.getElementsByClassName("club-select");
    if(cs){
        for(var i = 0; i < cs.length; i++){
            cs[i].addEventListener("change", addClubToGame, false);
        }
    }

    //saving the game
    var ag = document.getElementById("add-game");
    if(ag)
        ag.addEventListener("click", addGame, false);

    //edit picture
    var ep = document.getElementsByClassName("editPic")[0];
    if(ep){
        ep.addEventListener("change", editPicture, false);
    }
    ...
    if(tl){
        for(var i = 0; i < tl.length; i++){
            tl[i].addEventListener("click", openTab, false);
        }
    }

    //create fantasy club
    var fc = document.getElementById("fClub");
    if(fc){
        fc.addEventListener("click", createFClub, false);
    }

    //adding listeners to form inputs for player search
    var pi = document.getElementsByClassName("player-input");
    if(pi){
        for(var i = 0; i < pi.length; i++){
            if(pi[i].tagName == "INPUT" && pi[i].getAttribute("type") == "text")
                pi[i].addEventListener("keyup", searchPlayers, false);
            else
                pi[i].addEventListener("change", searchPlayers, false);
        }
    }

    //delete player from fantasy club
    var delfc = document.getElementsByClassName("player-delete-button");
    if(delfc){
        for(var i = 0; i < delfc.length; i++)
            delfc[i].addEventListener("click", deletePlayerFromFClub, false);
    }

    //fantasy club name change UI
    var nc = document.getElementById("club-name");
    if(nc){
        nc.addEventListener("click", openClubNameChange, false);
    }

    //setting listeners for drag&drop
    var cc = document.getElementsByClassName("content-cards");
    if(cc){
        var cards = [], j = 0;
        for(var i = 0; i < cc.length; i++){
            cards[j++] = cc[i].querySelectorAll(".card");
            cc[i].addEventListener("dragover", allowDragging, false);
            cc[i].addEventListener("drop", dropCard, false);
            cc[i].addEventListener("dragenter", dragOverEffectEnter, false);
            cc[i].addEventListener("dragleave", dragOverEffectLeave, false);
        }

        for(var i = 0; i < cards.length; i++){
            for(j = 0; j < cards[i].length; j++){
                //cards[i][j].addEventListener("dragover", allowDragging, false);
                cards[i][j].addEventListener("dragstart", dragCard, false);
                //cards[i][j].addEventListener("drop", dropCard, false);
            }
        }
